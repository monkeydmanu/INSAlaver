function toggleButtonsState(Numero_Machine, desactive) {
    const buttons = document.querySelectorAll(`[id^="bouton_temps_"][id$="_wallon_machine${Numero_Machine}"]`);
...
function addTimerButtonClickListeners(Numero_Machine) {
    ...
    buttons.forEach(button => {
        ...
        button.addEventListener("click", function() {
            ...
            const boutonPrincipalId = `bouton_machine${Numero_Machine}_wallon`;
    ...
    fetch('http://localhost:3000/timers')
        ...
        .then(data => {
            data.forEach(timer => {
                const machineButton = document.querySelector(`#${timer.machineId}`);
                const timerDisplay = document.querySelector(`#Afficheur_temps${timer.machineId.split("_")[1].slice(-1)}`);
                const bouton_annuler = document.querySelector(`#annuler_bouton_wallon_machine${timer.machineId.split("_")[1].slice(-1)}`);
...
document.querySelectorAll('[id^="annuler_bouton_wallon_machine"]').forEach((button, index) => {
    button.addEventListener("click", function() {
        const machineId = `bouton_machine${index + 1}_wallon`;
